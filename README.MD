...
operations** (each `execute_with_semaphore` completion counts as one). This
avoids hung `Page.goto` timeouts and empty search pages that build up when the
same browser process runs for weeks.
