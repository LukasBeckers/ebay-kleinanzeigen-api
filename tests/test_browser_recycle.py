...
def test_set_recycle_every_changes_threshold_at_runtime():
    async def _run():
        mgr = OptimizedPlaywrightManager(
        ...
        with patch("utils.browser.async_playwright") as ap:
            ...
            await mgr.start()
            mgr.set_recycle_every(2)
            assert mgr._recycle_every == 2
...
def test_set_recycle_every_rejects_non_positive():
    mgr = OptimizedPlaywrightManager(recycle_every=10)
    try:
        mgr.set_recycle_every(0)
